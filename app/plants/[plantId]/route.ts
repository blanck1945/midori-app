...
export async function GET(request: NextRequest, ctx: Ctx) {
  ...
  const plantRow = queryOne<Parameters<typeof mapPlant>[0]>(
  ...
  const diagRows = queryAll<Parameters<typeof mapDiagnosis>[0]>(
    'SELECT * FROM diagnoses WHERE plant_id = ? ORDER BY created_at DESC LIMIT 10',
    [plantId],
  )
  const taskRows = queryAll<Parameters<typeof mapTask>[0]>(
  ...
  const photoRows = queryAll<Parameters<typeof mapPlantPhoto>[0]>(
...
export async function DELETE(_request: NextRequest, ctx: Ctx) {
  ...
  const { plantId } = await ctx.params
  const result = run('DELETE FROM plants WHERE id = ? AND user_id = ?', [plantId, user.id])
  if (result.changes === 0) {
    return NextResponse.json({ message: 'Planta no encontrada' }, { status: 404 })
  }
